...
// startpunkt funzt schonma
/////////////////
routeFromLocation.addEventListener("click", function(){
  console.log(navigationStart);
  var stop = new Graphic({
    geometry: navigationStart.geometry, //.extent
    symbol: stopSymbol
  });
  routeLayer.add(stop);
  view.goTo(stop);
...
/*     var routeSymbol = {
    ...
    routeFromLocation.addEventListener("click",function(){
      ...
      routeParams.stops.features.push(navigationStart);
      routeParams.stops.features.push(navigationTarget);
      console.log("anzahl stops");
      console.log(routeParams.stops);
      
       routeTask.solve(routeParams).then(showRoute); 
    });
    function showRoute(data){
      console.log("route should show");
        var routeResult = data.routeResults[0].route;
        routeResult.symbol = routeSymbol;
        routeLayer.add(routeResult);
